namespace ecommerce_api.Servicios
{
    public class LocalFileStorage
    {
        ...
        public async Task<string> Almacenar(string contanedor,IFormFile archivo)
        {
            var extension = Path.GetExtension(archivo.FileName);
            var nombreArchivo = $"{Guid.NewGuid()}{extension}";
            string folder = Path.Combine(env.WebRootPath, contanedor);
            ...
            var urlArchivo = Path.Combine(url,contanedor,nombreArchivo).Replace("\\","/");
            return urlArchivo;
        }
